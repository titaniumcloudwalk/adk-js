...
import {traceAgentInvocation} from '../telemetry/tracing.js';
...
export abstract class BaseAgent {
  ...
  async *runLive(
    ...
    try {
      ...
      traceAgentInvocation({agent: this, invocationContext: ctx});

      // Handle before agent callback
      const beforeAgentEvent = await this.handleBeforeAgentCallback(ctx);
      if (beforeAgentEvent) {
        yield beforeAgentEvent;
      }
      if (ctx.endInvocation) {
        return;
      }

      // Run the live implementation
      for await (const event of this.runLiveImpl(ctx)) {
        yield event;
      }

      // Handle after agent callback
      const afterAgentEvent = await this.handleAfterAgentCallback(ctx);
      if (afterAgentEvent) {
        yield afterAgentEvent;
      }
    } finally {
      span.end();
    }
